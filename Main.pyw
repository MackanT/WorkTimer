...
def __get_customer_id(customer_name:str, date:str) -> int:
    ...
    customer_id_df = pd.read_sql(f"select customer_id from customers where customer_name = '{customer_name}' and '{date}' between valid_from and coalesce(valid_to, '2099-12-31')", conn)
...
def __get_project_id(project_name:str) -> int:
    ...
    project_id_df = pd.read_sql(f"select project_id from projects where project_name = '{project_name}'", conn)
...
def initialize_db(file_path:str) -> bool:
    ...
    def add_default_customers() -> None:
        
        if len(pd.read_sql("select * from customers", conn)) != 0:
            return
        
        
        
    def add_default_projects() -> None:
        
        if len(pd.read_sql("select * from projects", conn)) != 0:
            return
        
       

    def add_default_bonus() -> None:
        
        if len(pd.read_sql("select * from bonus", conn)) != 0:
            return
        
       

    def add_historic_time() -> None:
    ...
    def add_dates() -> None:
        ...
        date_table = pd.DataFrame({
            ...
            "iso_year": date_range.to_series().apply(lambda x: x.isocalendar().year),  # ISO year
            "month": date_range.month,                      # Month
            "week": date_range.to_series().apply(lambda x: x.isocalendar().week),      # ISO week
...
# Insert into tables
def insert_customer(customer_name:str, start_date:str, wage:int, valid_from:str) -> None:
    ...
    conn.execute("insert into customers (customer_name, start_date, wage, valid_from, valid_to, is_current, inserted_at, updated_at) values (?, ?, ?, ?, ?, 1, ?, ?)", 
                 (customer_name, start_date, wage, valid_from, None, now, None))
    # conn.commit()
    
def insert_time_row(customer_name:str, project_name:str, date:str = None, time:str = None, commit:bool = True) -> None:
    ...
    date_key = int(pd.read_sql(f"select * from dates where date = '{today}'", conn).iloc[0,0])
    ...
    if not rows or all(row[2] is not None for row in rows):  # No rows or all rows have end_time filled
        ...
        cost_df = pd.read_sql(f"SELECT wage FROM customers where customer_id = {customer_id}", conn)
        if cost_df.empty:
            cost = 0
        else:
            cost = int(cost_df.iloc[0,0])
        
        bonus_df = pd.read_sql(f"SELECT bonus_percent FROM bonus where '{date}' between start_date and coalesce(end_date, '2099-12-31')", conn)
        if bonus_df.empty:
            bonus = 0.0
        else:
...
def insert_project(project_name:str, customer_name:str, is_current:bool, commit:bool = True) -> None:
    
    customer_id = __get_customer_id(customer_name, datetime.now().strftime('%Y-%m-%d'))
    conn.execute("insert into projects (customer_id, project_name, is_current) values (?, ?, ?)", 
                 (customer_id, project_name, is_current))
    if commit:
        conn.commit()
    
def insert_bonus(bonus_percent:float, start_date:str, end_date:str=None, commit:bool = True) -> None:
    
    if not end_date:
        date_obj = datetime.strptime(start_date, '%Y%m%d')
        day_before = date_obj - timedelta(days=1)
        new_end_date = day_before.strftime("%Y-%m-%d")
        conn.execute(f"update bonus set end_date = '{new_end_date}' where end_date is Null")
        conn.commit()
    if len(start_date) == 8:
        start_date = __format_date(start_date)
        
    conn.execute("insert into bonus (bonus_percent, start_date, end_date) values (?, ?, ?)", 
...
def __add_customer_project(data:dict) -> tuple[bool, str]:
    ...
    customers = pd.read_sql(f"select * from customers where customer_name = '{c_name}'", conn)
    ...
    projects = pd.read_sql(f"select * from projects where project_name = '{p_name}' and customer_id = '{customer_id}'", conn)
    if len(projects[projects['is_current'] == 1]) > 0:
        ...
        conn.execute(f"update projects set is_current = 1 where project_id = {project_id}")
...
def __add_customer(data:dict) -> tuple[bool, str]:
    ...
    if len(customers[customers['is_current'] == 1]) > 0:
        ...
        insert_customer(c_name, s_date, wage, s_date)
        return (True, f'Customer {c_name} already exists in the database, updating wage!')
    
    elif len(customers[customers['is_current'] == 0]) > 0:
        conn.execute(f"update customers set is_current = 1 where customer_id = {customer_id}")
        conn.commit()
...
def __disable_customer(c_name: str) -> tuple[bool, str]:
    conn.execute(f"update customers set is_current = 0 where customer_name = '{c_name}'")
    conn.commit()
...
def __get_value(customer_name:str, project_name:str, date_key:int) -> float:
    ...
    date_data = pd.read_sql(f"select * from dates where date_key = '{date_key}'", conn).iloc[0]
    ...
    data.loc[(data['end_time'].isnull()) & (data['date_key'] == date_key), 'end_time'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    data['total_time'] = pd.to_datetime(data['end_time']) - pd.to_datetime(data['start_time'])
    ...
    if time_index == 0: # Today
        ...
        val = data[(data['iso_year'] == iso_year) & (data['week'] == iso_week)][sum_column].sum()
...
def __update_project(c_name:str, p_name:str, button:tk.Button) -> None:
    ...
    insert_time_row(c_name, p_name)
    current_color = button.cget('bg')
    color = __get_color('green') if current_color == __get_color('red') else __get_color('red')
...
def __run_sql(event, frame:tk.Toplevel, sql_input:tk.Entry, sql_output:ttk.Treeview) -> None:
    ...
    if 'delete' in sql_code or 'update' in sql_code:
        frame.grab_set()
        result = messagebox.askyesno("Confirm", "Running the SQL-command will make changes to the data do you want that?")
        frame.grab_release()
        if result:
            override = True
        else:
    ...
    try:
        if override:
            ...
            conn.commit()
            messagebox.showinfo("SQL-Command Successful", f"SQL command:\n\n{sql_code} ran successfully")
        else:       
...
def __update_value_id(val:int) -> None:
    ...
    cost_df = pd.read_sql(f"SELECT wage FROM customers where customer_id = {c_id}", conn)
    if cost_df.empty:
        cost = 0
    else:
        cost = int(cost_df.iloc[0,0])
    
    bonus_df = pd.read_sql(f"SELECT bonus_percent FROM bonus where '{start_time}' between start_date and coalesce(end_date, '2099-12-31')", conn)
    if bonus_df.empty:
        bonus = 0.0
    else:
        bonus = float(bonus_df.iloc[0,0])
...
def __get_active_projects() -> pd.DataFrame:
    ...
    projects = pd.read_sql("select c.customer_name, c.customer_id, p.project_name, p.project_id from projects p left join customers c on c.customer_id = p.customer_id where p.is_current = 1 and c.is_current = 1", conn)
...
def __entry_float_check(var: tk.Entry, p: str, *args):
    ...
    try:
        ...
        filtered_value = ''.join(
            c for i, c in enumerate(value) if c in valid_chars and 
            (c != '-' or i == 0) and 
            (c != '.' or value[:i].count('.') == 0)
        )
...
def __attempt_save_user_input(frame:tk.Toplevel, add_function, d_dict:dict=None, entity=None):       
    data = {}
...
### UI ###
def __create_popup(frame:tk.Toplevel, title:str, text:str, width:int=100, height:int=None) -> tk.Toplevel:
    
    if height:
        popup_window = tk.Toplevel(frame, bg=__get_color('beige'), width=width, height=height)
    else:
    ...
    label = tk.Label(popup_window, text=text, font=__get_font(14, True), bg=__get_color('beige'), fg=__get_color('teal'))
...
def __add_entry(frame:tk.Toplevel, text_var:tk.StringVar, text:str, width:int=16) -> tk.Entry:    
...
def __add_cancel(frame:tk.Toplevel) -> None:
...
def __add_save(frame:tk.Toplevel, text:str, f1, f2, entity=None, d_dict:dict=None) -> None:
    
    save_button = tk.Button(
        frame, 
        text=text, 
        command=lambda:f1(frame, f2, entity=entity, d_dict=d_dict),
        width=8, 
        height=1, 
        bg=__get_color('green'), 
        fg=__get_color('white'), 
...
def __add_bonus_popup():
    ...
    for config in entry_configs:
        ...
        entry = __add_entry(popup_window, text_var, t)
        d_dict[t] = {"entry": entry}

    __add_save(popup_window, "Add", __attempt_save_user_input, __add_bonus, entity=None, d_dict=d_dict)
...
def __update_value_popup():
    ...
    save_button = tk.Button(
        popup_window, 
        text="Update", 
        command=lambda:__update_value_id(int(entry.get())),
        width=8, 
        height=1, 
        bg=__get_color('green'), 
        fg=__get_color('white'), 
...
def __add_customer_popup():
    ...
    __add_save(popup_window, "Add", __attempt_save_user_input, __add_customer, entity=None, d_dict=d_dict)
    __add_cancel(popup_window)
    
def __remove_customer_popup():
        
    popup_window = __create_popup(root, "Remove Customer", "Select Customer to Remove")
    ...
    __add_save(popup_window, "Select", __attempt_remove_user_input, __disable_customer, entity=combobox, d_dict=None)
    __add_cancel(popup_window)
...
def __add_project_popup():
    ...
    d_dict = {}
    text = ["Project Name", "Customer Name"]
    for t in text:
        entry = __add_entry(popup_window, text_var=None, text=t)
        d_dict[t] = {'entry': entry}
    
    __add_save(popup_window, "Add", __attempt_save_user_input, __add_customer_project, entity=None, d_dict=d_dict)
    __add_cancel(popup_window)

def __remove_project_popup():            
    
    popup_window = __create_popup(root, "Remove Project", "Select Project to Remove")
    ...
    __add_save(popup_window, "Select", __attempt_remove_user_input, __disable_customer_project, entity=combobox, d_dict=data_dict)
    __add_cancel(popup_window)
...
def __add_sql_popup():
    ...
    h_scroll = ttk.Scrollbar(popup_window, orient="horizontal", command=sql_output.xview)
    h_scroll.pack(fill="x", side="bottom")
    sql_output.configure(xscrollcommand=h_scroll.set)
    sql_output.bind_all("<MouseWheel>", lambda event: sql_output.yview_scroll(int(-1*(event.delta/6)), "units"))
    sql_output.bind_all("<Shift-MouseWheel>", lambda event: sql_output.xview_scroll(int(-1*(event.delta/6)), "units"))


    
    
    entry.bind("<Return>", lambda event: __run_sql(event, popup_window, entry, sql_output))
    

          
def __add_project_button(c_name:str, p_name:str, state:bool, customer_index:int):
    
    col = __get_color('green') if state else __get_color('red')
    ...
    if customer_index == 0:
        ...
        c_label = tk.Label(this_frame, 
                          text=c_name, 
                          bg=__get_color('beige'),
                          fg=__get_color('teal'),
                          font=__get_font(13, True),
                          width=18,
                          height=1,
                          anchor=tk.W)
    ...
    button = tk.Button(this_frame, 
                       text=p_name, 
                       command=lambda c_name=c_name, p_name=p_name: __update_project(c_name, p_name, button), 
                       width=18, 
                       height=2, 
                       bg=col, 
                       fg='#ffffff',
                       font=__get_font(11, bold=True),
                       relief='flat'
                       )
    ...
    total_time = tk.Label(this_frame, 
                          text=formatted_value, 
                          bg=__get_color('beige'), 
                          fg=__get_color('teal'),
                          font=__get_font(11, False),
                          width=18,
                          height=2,
                          anchor=tk.W)
...
def __update_ui():
    ...
    current_times = pd.read_sql(f"select * from time where date_key = '{today_date}' and end_time is null", conn)
    active_projects = current_times[current_times['project_id'].isin(sorted_projects['project_id'])]
    ...
    for i, row in enumerate(sorted_projects.iterrows()):
        ...
        state = False
        if c_name in active_projects['customer_name'].tolist() and p_name in active_projects['project_name'].tolist():
            state = True
    ...
    def period_toggle_button():
        ...
        if time_index == 0:
            time_index = 1
            date_label.config({'text': 'Current Week'})
        elif time_index == 1: 
            time_index = 2
            date_label.config({'text': 'Current Month'})
        elif time_index == 2:
            time_index = 3
            date_label.config({'text': 'Current Year'})
        elif time_index == 3:
            time_index = 4
            date_label.config({'text': 'All Time'})
        elif time_index == 4:
            time_index = 0
    ...
    date_input = tk.Entry(frame_date_input, 
                      width=14,
                      font=__get_font(14),
    ...
    date_label = tk.Button(frame_date_input, 
                           text='Current Date', 
                           command=period_toggle_button, 
                           width=13,
                           bg=__get_color('green'), 
                           fg='#ffffff',
                           font=__get_font(14, bold=True),
                           relief='flat',
                        )
    date_label.grid(column=0, row=2, padx=5, pady=5)
    
    format_toggle = tk.Button(frame_date_input, 
                           text='0 h 0 min', 
                           command=format_toggle_button, 
                           width=8,  
                           bg=__get_color('green'), 
                           fg='#ffffff',
                           font=__get_font(14, bold=True),
