...
async def tasks_page():
    ...
    with ui.column().classes("w-full p-4"):
        ...
        # Tab panels
        with ui.tab_panels(tabs, value=list(customer_tabs.values())[0] if customer_tabs else None).classes("w-full"):
            for customer_name, tab in customer_tabs.items():
                with ui.tab_panel(tab):
                    await _render_customer_tasks(core, customer_name, tasks_config.get(customer_name, {}))


async def _render_customer_tasks(core: AppCore, customer_name: str, customer_config: dict):
    ...
    try:
        ...
        customer_projects = all_projects[
            all_projects["customer_name"] == customer_name
        ]
        ...
        for _, project in customer_projects.iterrows():
            ...
            with ui.expansion(project_name, icon="folder").classes("w-full mb-2"):
                ...
                if not project_tasks:
                    ui.label("No tasks configured for this project").classes("text-gray-500")
                else:
                    ...
                    with ui.column().classes("gap-2 p-2"):
                        for task in project_tasks:
                            ...
                            with ui.row().classes("items-center gap-2"):
                                ...
                                if task_desc:
                                    ui.label(f"- {task_desc}").classes("text-sm text-gray-500")

                        # Add new task button
                        ui.button("Add Task", icon="add", on_click=lambda: _show_add_task_dialog(core, project_id, project_name)).props("flat size=sm")
