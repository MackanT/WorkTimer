...
Run main_v2.py and navigate to http://localhost:8080/test
"""
...
async def test_page():
    ...
    with ui.column().classes('w-full p-8 gap-6'):
        ...
        with ui.card().classes('w-full'):
            ...
            ui.label('Open this page in multiple browsers/tabs. Each should have independent state.')
            
            with ui.row().classes('gap-4 items-center'):
                counter_input = ui.number(
                    label='Client Counter',
                    value=app.storage.user.get('counter', 0)
                ).classes('w-32')
        ...
        with ui.card().classes('w-full'):
            ...
            def trigger_event():
                message = f'Test event triggered at {datetime.now().strftime("%H:%M:%S")}'
        ...
        with ui.card().classes('w-full'):
            ...
            with ui.column().classes('gap-2'):
                ...
                ui.label(f'UI Config Keys: {list(core.ui_config.keys())[:5]}...').classes('text-mono')
