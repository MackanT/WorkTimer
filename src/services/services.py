    ...
    def run_in_thread(self, func, *args, **kwargs):
        ...
        def runner():
            try:
                ...
                self.event_bus.notify(f"Background task failed: {e}", type_="negative")
...
class DatabaseService(BaseService):
    ...
    async def get_projects(
        ...
        try:
            if customer_id:
                ...
                result = await self.core.query_engine.query_db(
                    query, params=(customer_id,)
                )
            else:
                query = "SELECT * FROM projects ORDER BY name"
                result = await self.core.query_engine.query_db(query)
            return result.to_dict("records") if hasattr(result, "to_dict") else []
        except Exception as e:
    ...
    def load_data_async(
        self, query: str, params: tuple = (), event_name: str = "data_loaded"
    ):
        """
        ...
        async def load():
            try:
                result = await self.core.query_engine.query_db(query, params=params)
                ...
                # Show notification
                self.event_bus.notify(f"Loaded {len(data)} records", type_="positive")
            except Exception as e:
                self.logger.error(f"Failed to load data: {e}")
                self.event_bus.notify(f"Failed to load data: {e}", type_="negative")
        ...
        async def refresh():
            try:
                self.event_bus.notify(
                    "Starting DevOps incremental refresh...", type_="info"
                )

                await self.core.devops_engine.update_devops(incremental=True)

                self.event_bus.notify(
                    "DevOps refresh completed successfully!", type_="positive"
                )
                ...
                self.event_bus.notify(f"DevOps refresh failed: {e}", type_="negative")
    ...
    def refresh_full_async(self):
        ...
        async def refresh():
            try:
                ...
                self.event_bus.notify(
                    "Full DevOps refresh completed successfully!", type_="positive"
                )
                ...
                self.event_bus.notify(
                    f"Full DevOps refresh failed: {e}", type_="negative"
                )

        return self.run_in_thread(refresh)

    async def create_work_item_async(
        self, item_type: str, title: str, customer_id: int, **kwargs
    ):
        """
        ...
        """
        try:
            self.event_bus.notify(f"Creating {item_type}...", type_="info")
            ...
            self.event_bus.notify(
                f"{item_type.capitalize()} created successfully!", type_="positive"
            )
            ...
            self.event_bus.notify(
                f"Failed to create {item_type}: {e}", type_="negative"
            )
            raise
...
class TimerService(BaseService):
    ...
    async def start_timer(
        ...
        try:
            ...
            self.event_bus.notify("Timer started!", type_="positive")
            ...
            self.event_bus.notify(f"Failed to start timer: {e}", type_="negative")
            raise
