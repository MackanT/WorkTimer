...
class DevOpsService(BaseService):
    ...
    async def refresh_incremental(self):
        ...
        if not self.core.devops_engine:
            self.event_bus.notify("DevOps not configured", type_="warning")
            return
        try:
            self.event_bus.notify("Starting DevOps incremental refresh...", type_="info")
            await self.core.devops_engine.update_devops(incremental=True)
            self.event_bus.notify("DevOps refresh completed successfully!", type_="positive")
    ...
    async def refresh_full(self):
        ...
        try:
            ...
            self.event_bus.notify("Full DevOps refresh completed successfully!", type_="positive")
            self.event_bus.emit("devops_refreshed")
        except Exception as e:
            self.logger.error(f"Full DevOps refresh failed: {e}")
            self.event_bus.notify(f"Full DevOps refresh failed: {e}", type_="negative")
