...
class AppCore:
    ...
    async def initialize_engines(self):
        ...
        try:
            ...
            self.query_engine = QueryEngine(
                file_name=self.settings.db_name,
                log_engine=db_logger
            )
            ...
            self.event_bus.notify(
                "WorkTimer initialized successfully!",
                type_="positive"
            )
    ...
    def get_or_create(cls, config_loader: Optional[ConfigLoader] = None) -> 'AppCore':
        ...
        Uses app.storage.user for per-client isolation.
        ...
        if 'app_core' in app.storage.user:
            return app.storage.user['app_core']
        ...
        app.storage.user['app_core'] = core
        return core
