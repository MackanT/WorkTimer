...
- **Minor improvements**
  ...
  - ad **Report a bug / Request a feature** — buttons in Settings → Data → About (and matching command-palette entries) open GitHub's new-issue form prefilled with the right label, a report template, and the environment line (app version, Docker vs direct) that reports always need. No tokens, no API — the user reviews and submits in the browser.
