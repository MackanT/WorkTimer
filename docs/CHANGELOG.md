...
- **Features**
  ...
  - fx hierarchy nodes rendering empty for titles with bracket-family characters - the "[ref:... | kst:...]" metadata tags are now stripped and any remaining () [] {} <> | " are removed (they are all mermaid label/shape delimiters)
