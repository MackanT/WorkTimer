...
### 5.0.5 (2026-xx-xx)
- **Features**
  - ad **Jira tracker module (read-only v1)** — `src/trackers/jira.py` implements the TrackerProvider contract against Jira Cloud REST v3 and registers as `jira` (it now appears in the customer Tracker dropdown). Reads work: connect/validate, issues → the canonical work-item frame (board, hierarchy, search, `#` find, time-linking), descriptions and comments (ADF → plain text), issue URLs. Mapping: issue types normalised by hierarchy level (Epic / Story / Sub-task — Bugs/Tasks show at Story level), board column = status name with done from the status category, named priorities → 1–4, numeric issue ids (keys shown in titles). Writes (create / field updates / board moves-as-transitions) and attachments are stubbed with honest "not implemented yet" messages — next iteration. Credentials v1: PAT field = `email:api-token`, Org URL = the `*.atlassian.net` site, project field = the project key. Live-test hardening: Jira board cards open their dialog with a status-based column dropdown (project statuses stand in for board columns); switching the board customer between trackers snaps the type chips to the new tracker's levels with its preferred one preselected (Story for Jira — its Sub-task leaf is auxiliary; User Story for DevOps) instead of blanking the board; a failed detail load can no longer leave the dialog on eternal skeletons (it clears, warns, and logs); one provider's fetch failure degrades to that customer only instead of blanking every connection; and the Git-ID picker's open-item filter is provider-neutral (excludes done-ish states rather than whitelisting Azure's Active/New, so Jira's To Do/In Progress items appear).
  - ad **Jira writes (v2)** — create issues (Epic / Story / Sub-task, resolving the project's real issue types by hierarchy level and parent links), update fields, and move cards. Board moves and state changes are Jira workflow *transitions*: a forbidden move fails with the allowed target statuses listed; "already there" is a no-op. The handler layer's Azure-era field names are translated (priority 1–4 ↔ Highest/High/Medium/Low, assignee display name → accountId via assignable-user search, Tags → labels with spaces dashed); a create rejected because optional fields aren't on the project's create screen (common in team-managed projects) retries once without exactly the rejected fields and says so. Descriptions and comments are written as minimal ADF (paragraphs + line breaks) — markdown syntax survives as text, and saving over a richly-formatted Jira description flattens it. The add work-item form is now tracker-aware: type options follow the selected customer's hierarchy with the tracker's preferred level pre-selected, parent options come from the levels above the chosen type, and Jira Stories share the User Story description template. Still Azure-only: attachments/image paste (off via capabilities), and the State dropdown's option list (a Jira status change that the workflow doesn't know is reported as a warning while the other fields still save — use the Board Column field to move Jira items).
  ...
  - ad work-item **descriptions are now searchable** — the sync already carried them (expand=All), so they're cached as stripped plain text (2000 chars) in the local devops table and included in both the board search and the palette's `#` find-mode. Existing installs: run a **Full Sync** once to populate them (the column auto-migrates; incremental only fills changed items).
  - ad **Shortcuts** tab on the Info page — all keyboard shortcuts and quick interactions (palette, `#` find, query-editor chords, timer right-click menus, board drag/drop) documented in one place.
- **Minor improvements**
  - ad **trackers are their own entity** — a tracker connection (type, org URL/site, PAT) now lives in a new `trackers` table with its own **Trackers** tab on the Data Input page (add / update / delete), and a customer just links to one via a Tracker dropdown on the customer forms. Rotating a PAT is now "update tracker", not "edit customer", and several customers can share one tracker (same org/PAT, each with their own project — the project stays per-customer). Existing installs migrate automatically at startup: each customer's embedded credentials become a tracker row (customers that shared identical credentials share one) and get linked; the legacy per-customer columns stay as a read-only fallback for unlinked customers and are no longer written by the forms. Deleting a tracker detaches its customers (they lose the connection until re-linked). Tracker changes re-initialize connections immediately, like customer credential changes did. Once every credentialed customer is linked, the legacy `customers.pat_token`/`org_url` columns are **dropped** at startup (guarded: an unlinked customer with legacy credentials blocks the drop, and SQLite < 3.35 leaves them in place) — history rows lose their old encrypted tokens by design, and the query page's default "customers" query is repaired to match. The default "projects" query now joins customers to show `customer_name` next to the id.
  - ad **the Data Input page is retired — entity management now lives in dialogs**. The Data Input nav button opens a dropdown (Customers / Trackers / Projects / Bonus); picking one opens a dialog right over whatever page is showing, with operation tabs (Add | Update | Disable | Re-enable) and the form's fields in a two-column grid (wide inputs span both, save bottom-right with the busy spinner). The palette's data commands ("Add customer", "Update tracker", …) open the same dialog directly on the right operation tab with its first field focused — no navigation at all. Everything stays config-driven from the same `add_data_page` section; the old page route is gone. The Time Tracker gets inline quick-adds: a dashed **"Add customer" ghost card** at the end of the customer row (board drop-zone look), and an **"＋ Add project" button under each customer card's divider** that opens the project dialog with that customer pre-filled (focus lands on the project name). A successful save closes the dialog, customers **without projects now show** as empty cards (ready for their first project), a saved customer/project **appears immediately** (the page now does a full rebuild on data changes — the old value-only update couldn't add cards), and **right-clicking a customer card's header** offers Update customer / Add project / Disable customer (disable acts immediately, no dialog — re-enable lives in the Customers dialog). Project rows' right-click menu gains **Update project** (dialog pre-filled with customer + project, for rename / default Git-ID) and **Disable project** (immediate). A customer rename now updates its card in place (the cached card order kept the old name), and the colour dots refresh on every rebuild; the dialog's operation tabs are centered and the colour field matches its siblings' height.
  - fx **blank band between the nav bar and the page content** — the nav-height CSS variable was corrected by a single requestAnimationFrame that races the frontend mount (first paint can beat Quasar's header render), leaving a too-large fallback; the sync now polls until the header exists, measures it directly, and re-measures on any header size change (timer pills, update badge).
  ...
  - fx **pages no longer hang for seconds while tracker connections (re)initialize** — opening any page used to await the full tracker init inline (internet check, org connects, startup sync) whenever it wasn't finished, which any customer/tracker save re-triggers; the init now runs in the background, pages render immediately with whatever state exists, and a `devops_refreshed` event wakes already-open pages when the connections land. The Board nav item is now shown based on a tracker being **configured** rather than live connectivity (the nav renders once per session, before background connections finish — it would otherwise vanish; no trackers configured still hides it). A board opened before the background init finishes reloads itself once when the data lands, instead of sitting on "no tracker data available" until a manual refresh.
  ...
  - ad palette **Add commands cover every connected tracker's levels** — the union across customers (Azure: Epic/Feature/User Story; Jira adds Story/Sub-task), each opening the add dialog preset to a customer whose tracker has that level (preferring the board's remembered customer).
  - ad palette **data-input shortcuts** — Add/Update/Disable/Re-enable customer, Add/Update/Delete tracker, project operations etc. (one command per configured entity operation) jump to the Data Input page with the right tab open and the form's first field focused, ready to type.
  - ad **PAT tokens are now encrypted at rest** — customer PATs in the database are Fernet-encrypted (`enc:` prefix) with a key auto-generated at `data/.pat_key` (git-ignored, and deliberately NOT part of backups: the backup copies only the database, so a backup restored on another machine has unreadable tokens — re-enter the PATs there; the log says so). Plaintext PATs from earlier versions are migrated once at startup; new/edited PATs encrypt on save, and the update-customer form shows the opaque `enc:` value instead of the real token (saving it back unchanged is a no-op, no double encryption). Requires the new `cryptography` dependency (`uv sync`); without it the app still runs and logs a plaintext warning.
  - ad the UI is **tracker-aware beyond the board**: the add-work-item form's State dropdown offers the customer's real states (Jira: its project statuses, cached; picked defaults snap per tracker like the type chips do), the hierarchy view's Focus selector and story-progress rollups follow the customer's hierarchy and working level (Jira: Epic/Story, "Done" counts by status), image-insert is hidden/refused for trackers without attachment support, and the card dialog's open-in link is labelled with the actual tracker name ("Open in Jira").
  - fx user-facing texts no longer claim "DevOps" for mixed trackers — sync toasts, empty-board/hierarchy notices, the settings tab ("Trackers"), the timer's "Tracker-ID"/"Store to tracker" controls and connection error messages now say tracker (internal names, tables and configs unchanged).
  - ad **Tracker selector** on the add/update-customer forms — makes the per-customer `integration_type` visible and editable (options come from the registered tracker providers, so a future Jira module appears automatically; currently the one option, defaulting to `devops`). A customer needing two trackers is modelled as two customer entries, one per tracker. Showing/hiding provider-specific credential fields per selection is deferred until a second provider exists.
