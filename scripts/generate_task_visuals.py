...
def get_customers_and_projects(db_path):
    ...
    try:
        ...
        cursor.execute("SELECT DISTINCT customer_name FROM tasks WHERE customer_name IS NOT NULL AND customer_name != ''")
        for row in cursor.fetchall():
            if row[0]:
                customers.add(row[0])
        
        # Get projects from tasks table  
        cursor.execute("SELECT DISTINCT project_name FROM tasks WHERE project_name IS NOT NULL AND project_name != ''")
        for row in cursor.fetchall():
            if row[0]:
                projects.add(row[0])
        ...
        try:
            cursor.execute("SELECT DISTINCT customer_name FROM customers WHERE customer_name IS NOT NULL")
            for row in cursor.fetchall():
                if row[0]:
                    customers.add(row[0])
                    
            cursor.execute("SELECT DISTINCT project_name FROM projects WHERE project_name IS NOT NULL")  
            for row in cursor.fetchall():
                if row[0]:
                    projects.add(row[0])
...
def generate_config(customers, projects, template_data):
    ...
    config = template_data.copy() if template_data else {
        'visual': {
    ...
    customer_icons = ['business', 'apartment', 'store', 'factory', 'domain', 'account_balance', 'corporate_fare']
    project_icons = ['code', 'web', 'mobile_friendly', 'cloud', 'storage', 'analytics', 'build', 'settings']
    colors = ['red', 'pink', 'purple', 'indigo', 'blue', 'light-blue', 'cyan', 'teal', 'green', 'light-green', 'orange', 'amber']
    ...
    if customers:
        ...
        for i, customer in enumerate(customers):
            if customer not in config['visual']['customers']:
                ...
                config['visual']['customers'][customer] = {
                    'icon': icon,
                    'color': color
                }
    ...
    if projects:
        ...
        for i, project in enumerate(projects):
            if project not in config['visual']['projects']:
                ...
                config['visual']['projects'][project] = {
                    'icon': icon,
                    'color': color
                }
...
def main():
    ...
    # Database path
    db_path = "data_dpg.db"  # Default database name
